...
class ResearchFile():
    def __init__(self, filename):
        ...
        self.packetOverviewDict = dict() #holds a dictionary with the amount of packets and teir types
    ...
    def buildConnectionSet(self, portWhiteList):
        ...
        for p in self.packets:
            ...
            if (((p.haslayer("IP")) & (not p.haslayer("DNS")) & (not p.haslayer("DHCP options")) & (
                    not p.haslayer("NTPHeader")) & (not p.haslayer("ICMP")))):  # checks if the packet uses the IP layer
                ...
                if not (
                        any(x.getAddress() == destIp.getAddress() for x in
                            self.allIps)):  # checks if IP is already listed
                    self.allIps.add(destIp)  # adds IP to the list for all IP adresses

                if not (
                        any(x.getAddress() == oriIp.getAddress() for x in
                            self.allIps)):  # checks if IP is already listed
        ...
        if counter != 0:
            print(counter,
                  "packets are not analyzed because they are DNS, NTP, ICMP or DHCP packets or didn´t have a IP Layer")
        if multicast != 0:
    ...
    def buildEntropyDict(self, upperLimit=9, lowerLimit=0):
        ...
        for packet in self.packets:
            ...
            if (packet.haslayer("TCP")):
                if ((packet.getlayer("TCP").flags == "A") or (packet.getlayer("TCP").flags == "S") or (
                        packet.getlayer("TCP").flags == "FA") or (packet.getlayer("TCP").flags == "SA") or (
                        packet.getlayer("TCP").flags == "RA")):
                    ackCounter += 1
            ...
            if ((packet.haslayer("Raw") & (not packet.haslayer("EAPOL")) & (not packet.haslayer("LLC")))):
                ...
                if (entropy(packet.getlayer('Raw').load) > lowerLimit and entropy(
                        packet.getlayer('Raw').load) < upperLimit):
                    try:
                        impText = packet.getlayer('Raw').load.decode().strip()
                        payload = re.sub(r'[\W+|_]', '',
                                         impText)  # \W+ deletes all special characters, \d+ deletes all numbers, |_ deletes all underscores
        ...
        self.packetOverviewDict['totalAmountPackets'] = amount
        #print("amount packets total:", amount)
        self.packetOverviewDict['amountNoRaw'] = amountRaw
        #print("amount no Raw level:", amountRaw)
        self.packetOverviewDict['amountByteBased'] = amountdonesemi
        #print("amount done byte based:", amountdonesemi)
        self.packetOverviewDict['amountDoneUtf'] = amountdone
        #print("amount done utf8 based:", amountdone)
        self.packetOverviewDict['amountDoneUtfExcluded'] = amountdoneExcluded
        #print("amount done utf8 based but the entropy is higher or lower then the given boundaries:", amountdoneExcluded)
        self.packetOverviewDict['amountExcludedPackets'] = ackCounter
        #print("packets used for ACK, SYN, SYN/ACK, FA:", ackCounter)
        self.packetOverviewDict['amountExcludedServices'] = arpCounter
        #print("packets used for ARP, DHCP or DNS:", arpCounter)
        self.packetOverviewDict['amountTotalProcessed'] = amountdone + amountdonesemi + amountRaw + amountdoneExcluded
        #print("total processed packets: ", amountdone + amountdonesemi + amountRaw + amountdoneExcluded)

    def printPacketDict(self):
        ''' a function just to print the packet list'''
        print("amount packets total:", self.packetOverviewDict['totalAmountPackets'])
        print("amount no Raw level:", self.packetOverviewDict['amountNoRaw'])
        print("amount done byte based:", self.packetOverviewDict['amountByteBased'])
        print("amount done utf8 based:", self.packetOverviewDict['amountDoneUtf'])
        print("amount done utf8 based but the entropy is higher or lower then the given boundaries:", self.packetOverviewDict['amountDoneUtfExcluded'])
        print("packets used for ACK, SYN, SYN/ACK, FA:", self.packetOverviewDict['amountExcludedPackets'])
        print("packets used for ARP, DHCP or DNS:", self.packetOverviewDict['amountExcludedServices'])
        print("total processed packets: ", self.packetOverviewDict['amountTotalProcessed'])
...
class Countdown():
    ...
    def runCountdown(self):
        self.count += 1
        if (((
                     100 / self.total) * self.count) > self.stage):  # if the amount done is over the value of stage it will print, stage starts with 10 and will increase by 10 every time
...
def prepareEntropyDic(dict, entropy, oriIp, destIp):
    ...
    if (oriIp in dict.keys()):
        if (destIp in dict[oriIp].keys()):
            ...
            dict[oriIp][destIp][
                "averageEntropy"] += entropy  # adds up the entropy which will be further calculated to the average within "finishEntropy" method
        else:
            dict[oriIp][destIp] = {"averageEntropy": entropy,
                                   "amount": 1}  # creates new dictionary entry if destination with corresponding origin IP is not in yet
    else:
        dict[oriIp] = {
            destIp: {"averageEntropy": entropy, "amount": 1}}  # creates new dictionary entry if origin IP is not in yet
...
class IpAddress():
    ...
    def lookupIP(self):
        ...
        url = 'http://api.ipstack.com/' + self.address + '?access_key=75ae5d89ba88b940232c4e8f26d6f7e8'.format(
            self.address)
...
class IotNetworkAnalyzer(Cmd):
    currentResearchFile = None
    ...
    parser.add_argument('-po', action="store_true", help='overview of ports', )
    parser.add_argument('-cn', action="store_true", help='overview of connections', )
    parser.add_argument('-cnlist', action="store_true", help='list of connections', )
    parser.add_argument('-ips', action="store_true", help='list of ips', )
    parser.add_argument('-g', action="store_true", help='overview of IPs and geolocation data', )
    parser.add_argument('-ct', action="store_true", help='overview of cleartext', )
    parser.add_argument('-e', action="store_true", help='calculated entropy values', )

    @cmd2.with_argparser(parser2)
    def do_load(self, args):
        if args.f:
            self.duration = MyTimer()  # creates a timestamp for the startingtime
            self.currentResearchFile = ResearchFile(args.f)  # holds a object with all information for a research
            self.currentResearchFile.buildConnectionSet(self.portWhiteList)

            print("Overview of IPs and their geolocation will now be created")
            self.currentResearchFile.buildLocationDict()  # holds a dictionaries with all IPs and their geolocation informations
            print("Done")

            print("Overview of IPs their send utf-8 cleartext and the destination IP will now be created")
            self.currentResearchFile.buildCleartextDict()  # holds a dictionary with all with all IP who sent cleartext with the corresponding destination address and the cleartext
            print("Done")

            print("the entropy dictionary will now be created")
            self.currentResearchFile.buildEntropyDict() # holds a dictionary with all IPs and their destinations and the average entropy of the exchanged packets
            print("Done")

            print("IoTNetworkAnalyzer: loading finished after:", self.duration.end())
            print("IoTNetworkAnalyzer: ***  you may use the 'ANALYSE' command to investigate the data or may use -h or help ***")

    @cmd2.with_argparser(parser)
    def do_analyze(self, args):
        if args.po:
            print("Overview of all used Ports and call amounts: \n",
                self.currentResearchFile.portDictionary)  # holds a dictionary used Ports and call amounts
        elif args.cn:
            print("Overview of all established connections: ")
            for c in self.currentResearchFile.connectionSet:  # holds a list with all established connections (destination IP, origin IP, call amounts, secure port usage)
                print(c.printAll())
        elif args.ips:
            print("Overview of all recoreded IPs (origin & destination): ")
            for i in self.currentResearchFile.allIps:  # holds a list with all IP-Addresses used
                print(i.getAddress())
        elif args.g:
            print("all IPs and their geolocation informations")
            print(self.currentResearchFile.locationDict)
        elif args.ct:
           print("Overview of IPs their send utf-8 cleartext and the destination IP: \n",
                  self.currentResearchFile.clearTextDictionary)
        elif args.e:
            print("Overview of IPs their destination and the average entropy: \n", self.currentResearchFile.entropyDict)
        elif args.pa:
            print("Overview of the analyzed packets: \n", self.currentResearchFile.packetOverviewDict)

def main():
    print("Welcome to the IoT Network Analyzer")
    app = IotNetworkAnalyzer()
